import { c as createComponent, $ as $$Font, b as $$Image } from './_astro_assets_BmSBloU0.mjs';
import 'piccolore';
import { c as createRenderInstruction, e as addAttribute, f as renderTemplate, u as unescapeHTML, m as maybeRenderHead, h as renderComponent, j as renderSlot, k as renderHead, l as isInputError } from './server_CqV_976N.mjs';
import { clsx } from 'clsx';
import { G as GOOGLE_SITE_VERIFICATION, B as BING_SITE_VERIFICATION, c as SITE_URL, d as actions } from './server_CHIeruv8.mjs';
...
const $$Header = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<header class="sticky top-0 z-40 w-full border-b border-slate-200 bg-white/95 backdrop-blur"> <div class="mx-auto flex max-w-7xl items-center justify-between px-6 py-3"> <a href="/" class="inline-flex items-center transition-transform duration-300 hover:scale-[1.03]" aria-label="Empower Kiribati home"> ${renderComponent($$result, "Image", $$Image, { "src": logoFullColor, "alt": "Empower Kiribati", "height": 56, "widths": [144, 216, 288], "sizes": "(min-width: 1024px) 216px, 168px", "class": "h-12 w-auto sm:h-14", "loading": "eager" })} </a> <nav class="hidden items-center gap-8 md:flex" aria-label="Primary"> ${nav.map((item) => renderTemplate`<a${addAttribute(item.href, "href")}${addAttribute(isActive(item.href) ? "page" : void 0, "aria-current")}${addAttribute([
    "relative text-sm font-medium transition-colors after:absolute after:inset-x-0 after:-bottom-1 after:h-0.5 after:origin-left after:scale-x-0 after:bg-lime-400 after:transition-transform after:duration-300 hover:after:scale-x-100",
    isActive(item.href) ? "text-slate-900 after:scale-x-100" : "text-slate-700 hover:text-slate-900"
  ], "class:list")}> ${item.label} </a>`)} </nav> <div class="flex items-center gap-3"> <a href="/#build-your-skills" class="hidden rounded-full bg-lime-300 px-5 py-2.5 text-sm font-semibold text-slate-900 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:bg-lime-400 hover:shadow-md hover:shadow-lime-400/40 sm:inline-flex">
Build your skills
</a> <button type="button" class="inline-flex items-center justify-center rounded-md p-2 text-slate-700 hover:bg-slate-100 md:hidden" aria-controls="empower-mobile-nav" aria-expanded="false" data-mobile-toggle> <span class="sr-only">Open menu</span> <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true"> <path stroke-linecap="round" stroke-linejoin="round" d="M4 6h16M4 12h16M4 18h16"></path> </svg> </button> </div> </div> <div id="empower-mobile-nav" class="hidden border-t border-slate-200 bg-white md:hidden"> <nav class="flex flex-col gap-1 px-6 py-4" aria-label="Mobile"> ${nav.map((item) => renderTemplate`<a${addAttribute(item.href, "href")} class="rounded-md px-3 py-2 text-base font-medium text-slate-700 hover:bg-slate-100 hover:text-slate-900"> ${item.label} </a>`)} <a href="/#build-your-skills" class="mt-2 inline-flex w-full items-center justify-center rounded-full bg-lime-300 px-5 py-2.5 text-sm font-semibold text-slate-900 hover:bg-lime-400 sm:hidden">
Build your skills
...
const $$Motion = createComponent(($$result, $$props, $$slots) => {
  return renderTemplate`${renderScript($$result, "/Users/tibereritobakanaan/Projects/ekwebsite/frontend/src/components/empower/Motion.astro?astro&type=script&index=0&lang.ts")}`;
...
const $$Layout = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "BaseLayout", $$BaseLayout, { "title": title, "description": description, "image": image, "imageAlt": imageAlt, "article": article, "noindex": noindex, "nofollow": nofollow, "includeOrgSchema": includeOrgSchema }, { "default": ($$result2) => renderTemplate`  ${renderSlot($$result2, $$slots["default"])}  ${renderComponent($$result2, "Motion", $$Motion, {})} `, "footer": ($$result2) => renderTemplate`${renderComponent($$result2, "Footer", $$Footer, { "slot": "footer" })}`, "header": ($$result2) => renderTemplate`${renderComponent($$result2, "Header", $$Header, { "slot": "header" })}` })}`;
