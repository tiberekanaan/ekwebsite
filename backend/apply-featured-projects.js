/**
 * One-off content migration: renames the programmes block heading from
 * "What we are running now" to "Featured projects". The items themselves are
 * untouched — the frontend now matches each item's title against the real
 * projects collection to render image cards linking to the project pages.
 ...
 *
 *   node apply-featured-projects.js
 *   STRAPI_URL=https://determined-strength-17a6de9eef.strapiapp.com STRAPI_API_TOKEN=xxx node apply-featured-projects.js
 */
...
function rebuild(block) {
  ...
  if (__component === "blocks.programmes") {
    return { __component, ...rest, title: TITLE };
  }
...
async function main() {
  ...
  if (!blocks.some((b) => b.__component === "blocks.programmes"))
    throw new Error("No blocks.programmes entry found on the homepage — nothing to rewrite");
  ...
  console.log(`Programmes block heading set to "${TITLE}".`);
}
