...
- When typing Strapi 5 API responses, remember that responses are now flattened by default (no `.attributes` wrappers) and entries are uniquely identified by a 24-character alphanumeric `documentId`.
...
- **Routing & Transitions:** Use the `<ClientRouter />` component for view transitions; the old `<ViewTransitions />` component has been removed in v6.
- Keep components focused - one job per component.

## Data Fetching & Content Management

- **Build-Time Fetching:** Fetch static content (Blogs, Pages) from Strapi using Astro's **Content Loader API** inside `src/content.config.ts`. 
- **Zod 4 Validation:** Validate all fetched CMS content and user inputs using Zod 4. Import `z` from `astro/zod` (not `astro:content`). Use new Zod 4 syntax like `z.email()` instead of the legacy `z.string().email()`.
- **No Legacy Globbing:** Use `import.meta.glob()` to load local files, as `Astro.glob()` has been completely removed in Astro 6.
...
- Components: `src/components/[feature]/ComponentName.astro` (or `.tsx`)
...
- Components: PascalCase (`ItemCard.astro`, `Button.tsx`)
...
- Access standard environment variables inline using `import.meta.env` as they are strictly inlined in v6; do not rely on `process.env` substitutions.
...
- Always return a safe, type-checked result from Actions and display user-friendly error messages via toast or UI banners.
